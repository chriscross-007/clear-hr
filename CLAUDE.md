...
- **Holiday Approvals routing (CLE-181)** — At submit time `submitHolidayBooking` calls `resolveProfileForBooking(memberId, absenceTypeId)`. If a profile exists, it picks the applicable Level 1 (Phase A only writes L1 — L2/L3 schema is in place but unwired), checks each main approver's "unavailable today" status (= an approved Holiday or Sick booking covering today), routes to mains when at least one is available else to delegates, writes a `booking_approvals` row, and sets `current_approval_level = 1`. `approveBooking` / `rejectBooking` verify the caller is in the active routed list (or the booking is legacy) before transitioning. List of approvers at a level: notify all, first-to-decide wins. Reject is terminal. Phase A out of scope: L2/L3 cascade, length-threshold UI, group approvers, non-Holiday seeded profiles. Spec: https://linear.app/clearhr/document/holiday-approvals-settled-spec-5a4138404dbb
