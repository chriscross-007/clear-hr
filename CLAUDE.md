...
**One-line rule for any new session:** if you're gating a page, an action, or a UI affordance, read the flag from the resolver. The legacy `members.role` / `members.permissions` columns were dropped in CLE-203 — the resolver's `EffectiveRights` object is the only access source.
...
#### Impl-siblings pattern for actions the mobile app also calls (CLE-227)
When a shipped web server action needs to be reachable from the mobile app via `/api/mobile/<domain>/*`, the auth model diverges: web uses Supabase cookies via `@/lib/supabase/server`; mobile uses `Authorization: Bearer <token>` verified against `admin.auth.getUser(token)`. Neither `createClient()` nor `resolveCaller()` in the action file works from a mobile API route.

The convention is a companion `<name>-actions-impl.ts` sibling next to the `"use server"` action file. The impl file is **not** `"use server"` — its exports stay regular module functions rather than becoming Server Actions — and it exports one `_myFunction(caller: DocumentsCaller, ...args)` per action-body that mobile wraps. The exported action becomes a ~5-line cookies-auth wrapper that delegates:

```ts
export async function myFunction(...args) {
  const caller = await buildDocumentsCallerFromCookies();
  if (!caller) return { success: false, error: "Not authenticated" };
  return _myFunction(caller, ...args);
}
```

The mobile route is a ~10-line bearer-auth wrapper over the same impl:

```ts
export async function GET(request: Request) {
  const caller = await buildDocumentsCallerFromBearer(request);
  if ("error" in caller) return NextResponse.json({...}, { status: caller.status });
  const result = await _myFunction(caller, ...);
  return NextResponse.json(result, { status: result.success ? 200 : ... });
}
```
...
Currently wrapped (all in `src/app/api/mobile/documents/*`):
- `/required`, `/other`, `/org`, `/traffic-light`, `/outstanding-acks` (GET reads)
- `/{id}`, `/{id}/signed-url`, `/{id}/acknowledge`, `/{id}/ack-status` (per-doc)
- `/upload` (multipart POST)

Impl siblings live at:
- `(dashboard)/my-documents/actions-impl.ts`
- `(dashboard)/documents/organisation/org-document-actions-impl.ts`
- `(dashboard)/documents/compliance-actions-impl.ts`
...
- **Admin Documents tab shape (CLE-222)** — The per-member Documents tab (`/members/[memberId]/docs`) renders two stacked cards mirroring the employee's own `/my-documents` shape: **Required documents** on top (the `<RequiredDocumentsCard>` from `members/[memberId]/employment/required-documents-card.tsx` — the file location predates the move and is retained), and **Other documents** below wrapping the existing Live / Trash lists in a shadcn `<Card>` with the Live/Trash toggle inside `<CardContent>`. Traffic-light click-through targets (`Employees Directory` Docs icon, Member sidebar avatar badge, `admin-dashboard` Documents-attention row) all point at `/members/{id}/docs#required-documents`. The Required Documents card owns the `#required-documents` anchor + spacer injection for scroll-to-top on hash arrival. The Employment tab no longer mounts `<RequiredDocumentsCard>`.
...
- **`member_org_document_hidden` (CLE-225)** — Junction table gating per-member visibility of organisation-scope documents. Fields: `organisation_id` (FK), `member_id` (FK), `document_id` (FK), `hidden_at`, `hidden_by_member_id` (FK, nullable, `ON DELETE SET NULL`). PK `(member_id, document_id)`. **Default semantics: visible.** Row presence = the pair is hidden. The `Viewable` checkbox on the admin's per-member Org Docs tab (`<MemberOrgDocumentsList>`) writes here via `setMemberOrgDocumentVisibility(memberId, documentId, viewable)`. Hidden pairs are subtracted from every consumer of the visibility set: `listOrgDocuments` (employee's own list), `getMyOutstandingAcknowledgements` (dashboard chip + outstanding list), `getDocumentCoverage` / `getDocumentAcknowledgementStatus` (per-doc coverage + Details dialog admin panel), `getOrgAcknowledgementCoverage` (Compliance Acknowledgements tab), and `remindOutstanding` (per-doc reminder audience). Existing `document_acknowledgement` rows for a pair are preserved for audit but become moot when the pair is hidden. Two FKs to `members` (`member_id`, `hidden_by_member_id`) — any join to `members` must use explicit `!column_name` hints. **RLS compromise (#gap):** SELECT/write policies are broad ("any authenticated user in the org"); the admin-only write gate is enforced at the server-action layer via `can_manage_organisation_documents`. Tightening RLS to `has_rights_flag(..., 'can_manage_organisation_documents')` on writes is a Tier-1 follow-up. Audit action: `document.visibility_changed` with `metadata.viewable` + `metadata.member_id` + `metadata.member_name`.
...
- `get_user_role(org_id)` — SECURITY DEFINER helper that returns `'employee'` when the caller's rights_profile rank is `'employee'`, `'admin'` otherwise, or NULL if the caller is not a member of the org. Sourced from `rights_profiles.rank`. Kept as a convenience for policies that just want "is this an admin at all". Prefer `has_rights_flag(org, '<flag>')` for anything more specific.
- `get_user_permission(org_id, permission_key)` — **Deprecated no-op.** Returns `false` for any key. Retained only as a safety net for any residual caller not caught by the CLE-203 policy sweep; new code must not call it.
- `get_user_team_id(org_id)` — SECURITY DEFINER helper returning the caller's `team_id` in the given org.
- `has_rights_flag(org_id, flag_name)` / `get_cross_user_access(org_id)` / `get_effective_tab_update(org_id, tab_key)` — the Path B RLS helpers introduced in `20260828000003`. These are the primitives every RLS policy now uses. All SECURITY DEFINER, all source from `rights_profiles`.
...
- `get_org_members()` — SECURITY DEFINER RPC. Sources rank + profile name from `rights_profiles` (rewritten in `20260828000002`). Returns the same 15-column shape the app has always consumed; `role` collapses to `'employee' | 'admin'`.
...
## UI Conventions
- **Save button gating (standing order):** In any dialog/sheet/form that edits an existing record, the primary Save (or Update / Apply) button is **disabled until the form data differs from the loaded initial state**. Simplest implementation is a `dirty` boolean computed as `JSON.stringify(payload) !== JSON.stringify(initial)` for the whole DTO, then `disabled={pending || !dirty || <any other required-field check>}`. In "create" mode this doesn't apply — any input on a blank form counts as a change, so the dirty check is skipped there (only the required-field guard remains). This applies to every edit form regardless of complexity — a user shouldn't be able to press Save on a form they haven't touched, and reopening a record should never present an eager Save.
...
- **Date/time formatting:** UK locale, always. Never show raw ISO strings (`2026-09-02` or `2026-09-02T14:30:00Z`) to a user — those are storage format, not display format.
  - **Date + time:** `toLocaleDateString("en-GB", { day: "2-digit", month: "short", year: "numeric", hour: "2-digit", minute: "2-digit", hour12: false })` → `"02 Sep 2026, 14:30"`.
  - **Date only:** `toLocaleDateString("en-GB", { day: "2-digit", month: "short", year: "numeric", timeZone: "UTC" })` → `"02 Sep 2026"`. Always pass `timeZone: "UTC"` for date-only values so a UK caller doesn't see the date shift by a day depending on server time.
  - Do not pull in `date-fns` just for formatting.
  - Any new render surface that shows a date must go through one of these two patterns. Applies equally to the audit page, where `changes` and `metadata` diffs can carry stored ISO strings that need formatting to display.
...
- **`rights_profiles`** — one row per named profile per org. Carries:
  ...
  - `rank` — vestigial metadata (CLE-197 flattened the UI; users never see rank). Retained because the seed defaults + guard triggers + `get_user_role` still key on it. Not user-visible anywhere.
...
### Legacy plumbing removed (CLE-203)

The CLE-196 → CLE-201 cutover deliberately stopped short of dropping the old plumbing so the RLS rewrites could be tackled separately. CLE-203 finished the job:

- **Dropped columns:** `members.role`, `members.permissions`, `members.admin_profile_id`, `members.employee_profile_id`.
- **Dropped tables:** `admin_profiles`, `employee_profiles`.
- **Rewritten policies:** ~40 residual RLS policies that still read `members.role` directly or called `get_user_permission` were rewired onto `has_rights_flag` / `get_cross_user_access` / `get_user_team_id`. See `20260903000001_cle_203_rls_rewrite_and_drop.sql` for the full policy-by-policy mapping and the intent-to-flag choices.
- **`get_user_permission` neutralised:** kept as a no-op returning `false` (safety net for any caller a policy-grep might have missed). Do not add new callers.
- **The 9 `members` policies collapsed into 4:** `members_read_by_scope` (SELECT), `members_admin_insert` / `members_admin_update` / `members_admin_delete`. The 4 dead "Owners can *" policies (always-false since Path B) were dropped along the way.
- **`src/app/(dashboard)/settings/profiles/rights/rights-profiles-client.tsx`** was stubbed during CLE-201 and can be deleted from Windows Explorer when convenient.
...
- **Flash notice.** The dashboard layout renders a `<FlashNotice>` primitive (`src/components/ui/flash-notice.tsx`) that reads `?flash=…&flashTone=…` from the URL, shows a small bottom-right pill for 4s, then strips the query params. Any client-side navigation can append these params to fire a one-shot message. Currently used by the view-mode toggle to confirm the switch. This is a zero-dep stand-in — swap for `sonner` if we want a proper multi-message toast pipeline (see `flash-notice.tsx` for the swap notes).
