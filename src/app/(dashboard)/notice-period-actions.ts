...
async function getCallerOwner() {
  ...
  // CLE-196b-5 — Notice-period profile CRUD is org-level config.
...
export async function setMemberNoticeProfile(
  ...
  try {
    const { supabase, member } = await getCallerMember();
    // CLE-196b-5 — Assigning a notice-period profile to a member requires
    // Personal-tab update rights (or org-settings for admins).
    const { getEffectiveRightsForUser } = await import("@/lib/rights-resolver");
    const { data: { user } } = await supabase.auth.getUser();
    const resolvedNP = user ? await getEffectiveRightsForUser(user.id) : null;
    const writeAccessNP =
      resolvedNP?.rights.tabs.personal?.update === true ||
      resolvedNP?.rights.canEditOrgSettings === true;
    if (!writeAccessNP) return { success: false, error: "Insufficient permissions" };
