# Holiday Management — Feature Specification

## Overview

Holiday management for ClearHR allows organisations to configure and manage employee time off. The system is designed to be flexible: organisations choose how holidays are measured, how they're allocated, and whether employees request approval or book directly.
...
| **Holiday year** | The period over which entitlement is measured. Typically Jan–Dec or Apr–Mar, configurable per org. |
| **Leave type** | A category of absence (e.g. Annual Leave, Sick, Compassionate, TOIL). Each may have its own entitlement and rules. |
...
These are org-level settings that shape how the entire holiday system behaves. Each dimension is independent.
...
**Decision needed:** Can an org switch between days and hours after setup, or is it locked at creation?

### Allocation: Fixed vs Flexible (Accrual)

| Setting | Fixed | Flexible (Accrual) |
|---------|-------|---------------------|
...
**Decision needed:** Allow booking against future accrual (i.e. borrow ahead), or restrict to accrued balance only?
...
**Decision needed:** Can the workflow be set per leave type (e.g. Annual Leave requires approval, TOIL is self-service)?
...
| Applies to | All leave types / Specific types only |
